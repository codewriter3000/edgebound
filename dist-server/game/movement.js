...
export function buildOccupancyMap(pieces) {
    const map = new Map();
...
function buildMoveCandidates(piece, origin) {
    const candidates = [];
    ...
    for (let step = 1; step <= maxStep; step += 1) {
        ...
        if (isHorizontalLine) {
            candidates.push({ dx: delta, dy: 0, endKinds: ['square'] }, { dx: -delta, dy: 0, endKinds: ['square'] }, { dx: 1, dy: delta, endKinds: ['square'] }, { dx: -1, dy: delta, endKinds: ['square'] }, { dx: 1, dy: -delta, endKinds: ['square'] }, { dx: -1, dy: -delta, endKinds: ['square'] });
        }
        else if (isVerticalLine) {
            candidates.push({ dx: 0, dy: delta, endKinds: ['square'] }, { dx: 0, dy: -delta, endKinds: ['square'] }, { dx: delta, dy: 1, endKinds: ['square'] }, { dx: delta, dy: -1, endKinds: ['square'] }, { dx: -delta, dy: 1, endKinds: ['square'] }, { dx: -delta, dy: -1, endKinds: ['square'] });
        }
        else {
            candidates.push({ dx: delta, dy: 0, endKinds: ['square'] }, { dx: -delta, dy: 0, endKinds: ['square'] }, { dx: 0, dy: delta, endKinds: ['square'] }, { dx: 0, dy: -delta, endKinds: ['square'] }, { dx: 1, dy: delta, endKinds: ['square'] }, { dx: -1, dy: delta, endKinds: ['square'] }, { dx: 1, dy: -delta, endKinds: ['square'] }, { dx: -1, dy: -delta, endKinds: ['square'] }, { dx: delta, dy: 1, endKinds: ['square'] }, { dx: delta, dy: -1, endKinds: ['square'] }, { dx: -delta, dy: 1, endKinds: ['square'] }, { dx: -delta, dy: -1, endKinds: ['square'] });
        }
    }
    return candidates;
}
function respectsFriendlySpacing(nextSpot, movingPiece, pieces, isPickAction) {
    if (isPickAction) {
        return true;
    }
...
export function computeValidMoveTargets({ phase, actionMode, actionsUsed, turn, selectedPiece, actedPieceIds, pieces, occupancy, blockedPickPointIds, }) {
    const targets = new Set();
    if (phase !== 'play' || selectedPiece == null || actionMode !== 'move') {
        return targets;
    }
    if (actionsUsed >= MAX_MOVES_PER_TURN ||
    ...
    candidates.forEach(({ dx, dy, endKinds }) => {
        ...
        if ((selectedPiece.type === 'square' || selectedPiece.type === 'circle') &&
...
export function computeValidPickTargets({ phase, actionMode, turn, selectedPiece, actedPieceIds, pieces, occupancy, blockedPickPointIds, }) {
    const targets = new Set();
    if (phase !== 'play' || selectedPiece == null || actionMode !== 'pick') {
        return targets;
    }
    if (selectedPiece.owner !== turn ||
        selectedPiece.locked ||
        actedPieceIds.includes(selectedPiece.id)) {
        return targets;
    }
    ...
    pieces.forEach((piece) => {
        ...
        if (selectedPiece.type === 'triangle') {
            ...
            const isOrthogonal = (dx > 0 && dx <= pickDelta && dy === 0) ||
