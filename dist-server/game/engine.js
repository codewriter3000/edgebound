import { LATTICE_MAX, MAX_MOVES_PER_TURN, PIECE_LIMITS, SHAPES, } from './constants';
import { SPOT_BY_ID } from './board';
import { buildOccupancyMap, computeValidMoveTargets, computeValidPickTargets, } from './movement';
import { canPlaceInSetup, hasRequiredSetupSpacing, otherPlayer, } from './rules';
...
function registerAction(state, updatedPieces, actorId, newPickPointId) {
    const maybeWinner = checkWin(updatedPieces);
    const nextPickPointIds = newPickPointId == null || state.pickPointIds.includes(newPickPointId)
        ? state.pickPointIds
...
function applySetupPlacement(state, actor, pieceType, spotId) {
...
function applyMove(state, actor, pieceId, targetSpotId) {
    if (state.phase !== 'play') {
    ...
    const updatedPieces = state.pieces.map((piece) => piece.id === selectedPiece.id ? { ...piece, spotId: targetSpot.id, hasMoved: true } : piece);
    return {
        accepted: true,
        state: registerAction(state, updatedPieces, selectedPiece.id),
    };
}
function applyPick(state, actor, pieceId, targetPieceId) {
    if (state.phase !== 'play') {
        return { accepted: false, state, error: 'Game is not in play phase.' };
    }
    if (state.turn !== actor) {
...
function applyEndTurn(state, actor) {
    if (state.phase !== 'play') {
        return { accepted: false, state, error: 'Game is not in play phase.' };
    }
    if (state.turn !== actor) {
...
export function applyGameAction(state, actor, action) {
